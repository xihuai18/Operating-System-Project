...
rem gcc -Wa,-R -march=i386 -g -m32 -mpreferred-stack-boundary=2 -ffreestanding -c .\user\testMultiTerminal.c -o .\user\testMultiTerminal.o > log.txt
rem ld -m i386pe -N .\user\testMultiTerminal.o .\user\std.o  -T linkscript -o .\user\testMultiTerminal.tmp > log.txt
rem objdump.exe -Sl .\user\testMultiTerminal.tmp -M intel -m i8086 > terminal.txt
rem objcopy -O binary .\user\testMultiTerminal.tmp .\user\testMultiTerminal.bin > log.txt
...
dd if=.\lib\text.txt of=..\imgs\boot.img bs=512 seek=26 count=1 conv=notrunc
...
rem dd if=.\user\testMultiTerminal.bin of=..\imgs\boot.img bs=512 seek=198  count=54 conv=notrunc
