...
installInt21h:
	...
	mov word [cs:int21h_store_ebx], bx
	...
	int21hexitprg, int21hwait, int21hfork, \
	int21hMalloc, int21hGetSem, int21hFreeSem, \
	int21hP, int21hV, int21hDelay, \
	int21hFree 
	times 0x98-($-int21hTable) db 0
	dw int21hexit
	int21h_store_ip dw 0
	int21h_store_cs dw 0
	int21h_store_flags dw 0
	int21h_store_ebx dd 0
	int21_store_retaddr dw 0
...
int21hexitprg:
	...
	push dword [cs:int21h_store_ebx]
...
int21hfork:
	;; flags\cs(进程)\ip(进程)\ip(int21h)
	; pop word [cs:int21_store_retaddr]
	; push word [cs:int21h_store_flags]
	; push word [cs:int21h_store_cs]
	; push word [cs:int21h_store_ip]
	cli
	; pop word [cs:int21hfork_store_ip]
	; pop word [cs:int21hfork_store_cs]
	; pop word [cs:int21hfork_store_flags]

	; push word [cs:int21hfork_store_ip]
	push cs

	;进入后，ip(进程)\cs(进程)\ip(调度函数或中断,应舍弃)
	push bx
    push ax
    push ds
    mov ax, cs
 ...
 ;    xor eax, eax
	...
	int21hfork_store_cs dw 0
	int21hfork_store_ip dw 0
	int21hfork_store_flags dw 0
...
installInt16h:
    ...
    mov ax, word [es:16*4+2]
    mov [cs:tmp16+2], ax
    mov ax, word [es:16*4]
    mov [cs:tmp16], ax

    cli
    mov word [es:16*4+2], cs
    mov word [es:16*4], int16h
    sti
    pop ds
    pop es
    popa
...
extend00hIn16h:
	...
	pop word [cs:tmpinInt16h]
	pop word [cs:tmpinInt16h]
	jmp int16ret

	nocurPage00h:
	; 此时，栈里还有ax, 标志寄存器，返回地址，标志寄存器
	pop word ax
	pop word [cs:tmpinInt16h]
	pop word [cs:tmpinInt16h] ;ip
	mov word [cs:tmp2inInt16h], ax
	mov ax, [cs:tmpinInt16h]
	sub ax, 2
	push ax
	mov ax, word [cs:tmp2inInt16h]
...
extend01hIn16h:
	...
	pop word [cs:tmpinInt16h]
	pop word [cs:tmpinInt16h]
	pop word [cs:tmp2inInt16h]
	mov [cs:tmp3inInt16h], ax
	pop ax
	or ax, 1
	push ax
	mov ax, [cs:tmp3inInt16h]
	push word [cs:tmp2inInt16h]
	push word [cs:tmpinInt16h]
	jmp int16ret
...
int8hret:
	...
	mov dx, 0x1805
	mov bl, 0fh
	mov bh, [cs:_curPageId]
	mov ah, 13h
	mov al, 0
	int 10h

	mov ax, [cs:_curPageId]
	add ax, 0x30
	mov [cs:pageId], ax
	mov bp, pageId
	mov cx, 1
...
_loadReal:
    ...
    mov es, ax                ;设置段地址（不能直接mov es,段地址）
 	...
 	; 计算扇区
 	mov ax, word [esp+0x14] ;lma
 	mov dx, 0
 	mov cx, 512
 	div cx ;现在ax里面是相对扇区号
 	mov cx, 18
 	div cx ; 余数在dx里面，ax现在是商
 	mov cl, dl
 	inc cl ; 起始扇区号确定
...
;;;; void memcpy(int segOfSour, int sour, int segOfDes, int des, int size);;;
	...
	mov ds, ax
	mov si, word [ss:ebp+12]
	mov ax, word [ss:ebp+16]
	mov es, ax
	mov di, word [ss:ebp+20]

	mov cx, word [ss:ebp+24]
